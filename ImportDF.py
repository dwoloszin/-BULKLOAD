...
def ImportData(fields, pathImport):
    fileData = datetime.fromtimestamp(getmtime(pathImport)).strftime('%Y%m%d')
    iter_csv = pd.read_csv(pathImport, index_col=None, encoding="ANSI",header=0, error_bad_lines=False,dtype=str, sep = ';',iterator=True, chunksize=10000, usecols = fields )
    df = pd.concat([chunk for chunk in iter_csv]) # & |  WORKS
    frameSI = df[fields] # ordering labels 
    frameSI["dataArchive_Import"] = fileData   
    frameSI = frameSI.drop_duplicates()
    return frameSI